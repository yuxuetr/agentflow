...
use crate::nodes::conditional::{ConditionalNode, ConditionType};
...
impl NodeFactory for LlmNodeFactory {
    fn create_node(&self, config: ResolvedNodeConfig) -> NodeResult<Box<dyn AsyncNode>> {
        ...
        if let Some(temp) = config.parameters.get("temperature").and_then(|v| v.as_f64()) {
...
impl NodeFactory for HttpNodeFactory {
    fn create_node(&self, config: ResolvedNodeConfig) -> NodeResult<Box<dyn AsyncNode>> {
        let url = config.parameters.get("url")
            .and_then(|v| v.as_str())
...
impl NodeFactory for FileNodeFactory {
    fn create_node(&self, config: ResolvedNodeConfig) -> NodeResult<Box<dyn AsyncNode>> {
        let operation = config.parameters.get("operation")
            .and_then(|v| v.as_str())
            .unwrap_or("read");
...
impl NodeFactory for TemplateNodeFactory {
    fn create_node(&self, config: ResolvedNodeConfig) -> NodeResult<Box<dyn AsyncNode>> {
        let template = config.parameters.get("template")
            .and_then(|v| v.as_str())
            .ok_or_else(|| NodeError::ValidationError {
        ...
        if let Some(format) = config.parameters.get("output_format").and_then(|v| v.as_str()) {
            node = node.with_format(format);
        }
        
        // Set variables if specified
        if let Some(vars_obj) = config.parameters.get("variables").and_then(|v| v.as_object()) {
...
impl NodeFactory for BatchNodeFactory {
    fn create_node(&self, config: ResolvedNodeConfig) -> NodeResult<Box<dyn AsyncNode>> {
        let items_key = config.parameters.get("items_key")
            .and_then(|v| v.as_str())
            .unwrap_or("items");
        ...
        if let Some(max_concurrent) = config.parameters.get("max_concurrent").and_then(|v| v.as_u64()) {
...
impl NodeFactory for ConditionalNodeFactory {
    fn create_node(&self, config: ResolvedNodeConfig) -> NodeResult<Box<dyn AsyncNode>> {
        let condition = config.parameters.get("condition")
            .and_then(|v| v.as_str())
            .ok_or_else(|| NodeError::ValidationError {
        ...
        } else if let Some(threshold) = config.parameters.get("greater_than").and_then(|v| v.as_f64()) {
