...
impl From<NodeError> for AgentFlowError {
  fn from(err: NodeError) -> Self {
    match err {
      NodeError::ConfigurationError { message } => AgentFlowError::NodeExecutionFailed {
        message: format!("Configuration error: {}", message),
      },
      NodeError::ExecutionError { message } => AgentFlowError::AsyncExecutionError { message },
      NodeError::ValidationError { message } => AgentFlowError::NodeExecutionFailed {
        message: format!("Validation error: {}", message),
      },
      NodeError::CoreError(core_err) => core_err,
      NodeError::IoError(io_err) => AgentFlowError::AsyncExecutionError {
        message: format!("I/O error: {}", io_err),
      },
      NodeError::SerializationError(ser_err) => {
        AgentFlowError::SerializationError(ser_err.to_string())
      }
      NodeError::HttpError { message } => AgentFlowError::AsyncExecutionError {
        message: format!("HTTP error: {}", message),
      },
      NodeError::FileOperationError { message } => AgentFlowError::AsyncExecutionError {
        message: format!("File operation error: {}", message),
      },
      NodeError::Base64Error(b64_err) => AgentFlowError::AsyncExecutionError {
