...
impl BatchNode {
  ...
  async fn process_batch(
    ...
    items: &[Value],
    shared: &SharedState,
  ) -> Result<Vec<Value>, AgentFlowError> {
    ...
    if let Some(child_node) = &self.child_node {
      ...
      for (index, item) in items.iter().enumerate() {
        ...
        tasks.spawn(async move {
          ...
          child_clone
            .post_async(&shared_clone, prep_result, exec_result.clone())
            .await?;
...
impl AsyncNode for BatchNode {
  ...
  async fn exec_async(&self, prep_result: Value) -> Result<Value, AgentFlowError> {
    ...
    let batch_size = prep_result["batch_size"]
      .as_u64()
      .unwrap_or(self.batch_size as u64) as usize;
