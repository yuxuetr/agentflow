...
mod tests {
  ...
  async fn test_http_node_post_async() {
    ...
    assert_eq!(
      shared.get("post_test_status").unwrap(),
      exec_result["status"]
    );
  ...
  async fn test_http_node_method_variations() {
    ...
    for method in methods {
      let node = HttpNode::new("method_test", "https://httpbin.org/get").with_method(method);
  ...
  fn test_http_node_unsupported_method() {
    ...
    let node = HttpNode::new("invalid_method", "https://httpbin.org/get").with_method("INVALID");
  ...
  async fn test_http_node_with_custom_headers() {
    ...
    let node =
      HttpNode::new("header_test", "https://httpbin.org/headers").with_headers(headers.clone());
    ...
    assert_eq!(
      result_headers["X-Custom-Header"].as_str().unwrap(),
      "test-value"
    );
    assert_eq!(
      result_headers["User-Agent"].as_str().unwrap(),
      "AgentFlow/0.1.0"
    );
  }
