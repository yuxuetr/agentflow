...
impl AsyncNode for FileNode {
    ...
    async fn exec_async(&self, prep_result: Value) -> Result<Value> {
        ...
        match operation {
            ...
            "write" => {
                ...
                let content = prep_result["content"].as_str().ok_or_else(|| {
                    AgentFlowError::AsyncExecutionError {
            ...
            "append" => {
                ...
                tokio::fs::write(path, &existing_content).await.map_err(|e| {
                    AgentFlowError::AsyncExecutionError {
    ...
    async fn post_async(
        ...
        if let Some(content) = exec_result["content"].as_str() {
            shared.insert(format!("{}_content", self.name), Value::String(content.to_string()));
        }

        // Store size
        if let Some(size) = exec_result["size"].as_u64() {
            shared.insert(format!("{}_size", self.name), Value::Number(serde_json::Number::from(size)));
        }
