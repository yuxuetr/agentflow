use crate::{AsyncNode, SharedState, NodeResult};
...
impl TemplateNode {
    ...
    fn render_template(&self, template: &str, context: &HashMap<String, Value>) -> NodeResult<String> {
...
impl AsyncNode for TemplateNode {
    ...
    async fn exec_async(&self, prep_result: Value) -> std::result::Result<Value, AgentFlowError> {
        ...
        let output_format = prep_result["output_format"].as_str().unwrap_or(&self.output_format);
        let context = prep_result["context"].as_object()
    ...
    async fn post_async(
        ...
        println!("💾 Stored template output in shared state as: {}", output_key);
        Ok(None)
    }
...
mod tests {
    ...
    async fn test_template_node_simple_rendering() {
        ...
        assert_eq!(output.as_str().unwrap(), "Hello Alice, welcome to AgentFlow!");
        
        let rendered_text = shared.get("rendered_text").unwrap();
        assert_eq!(rendered_text.as_str().unwrap(), "Hello Alice, welcome to AgentFlow!");
    }
    ...
    async fn test_template_node_numeric_values() {
        ...
        let shared = SharedState::new();
        shared.insert("count".to_string(), Value::Number(serde_json::Number::from(42)));
        shared.insert("price".to_string(), Value::Number(serde_json::Number::from_f64(19.99).unwrap()));
    ...
    async fn test_template_node_boolean_values() {
        let node = TemplateNode::new("bool_test", "Active: {{is_active}}, Complete: {{is_complete}}");
        let shared = SharedState::new();
    ...
    async fn test_template_node_json_output_format() {
        let node = TemplateNode::new("json_test", r#"{"message": "Hello {{name}}"}"#)
            .with_format("json");
    ...
    async fn test_template_node_invalid_json_fallback() {
        let node = TemplateNode::new("invalid_json", "This is not JSON: {{value}}")
            .with_format("json");
    ...
    async fn test_template_node_yaml_output_format() {
        ...
        assert_eq!(output.as_str().unwrap(), "message: Hello YAML\nstatus: active");
    }
    ...
    async fn test_template_node_prep_async() {
        ...
        let shared = SharedState::new();
        shared.insert("shared_var".to_string(), Value::String("shared_value".to_string()));

        let prep_result = node.prep_async(&shared).await.unwrap();
        
        assert_eq!(prep_result["template"].as_str().unwrap(), "Template: {{value}}");
...
"#;
        ...
        shared.insert("count".to_string(), Value::Number(serde_json::Number::from(100)));
        shared.insert("success_rate".to_string(), Value::Number(serde_json::Number::from_f64(95.5).unwrap()));
