...
describe('schemas — happy paths', () => {
  it('RunRecord accepts the canonical server shape', () => {
  ...
  it('RunRecord allows missing optional fields', () => {
  ...
  it('StreamedEvent rejects seq as a string', () => {
  ...
  it('HarnessSession round-trips the canonical server shape', () => {
  ...
  it('HarnessEvent rejects missing session_id', () => {
  ...
  it('DiagnosticsReport accepts minimal status-only shape', () => {
    const parsed = DiagnosticsReportSchema.parse({ status: 'ok' });
    assert(parsed.status === 'ok', 'status preserved');
  });

  it('DiagnosticsReport rejects unknown status value', () => {
...
describe('schemas — unknown-field passthrough', () => {
  it('RunRecord passthrough preserves unknown fields', () => {
    const parsed = RunRecordSchema.parse({
      id: 'r',
      workflow: 'w',
      status: 's',
  ...
  it('HarnessSession passthrough preserves unknown fields', () => {
    const parsed = HarnessSessionSchema.parse({
      id: 's',
      tenant_id: 't',
      status: 's',
...
describe('parseJson + SchemaValidationError', () => {
  it('parseJson returns the validated value on success', () => {
  ...
  it('parseJson throws SchemaValidationError on shape mismatch', () => {
  ...
  it('SchemaValidationError truncates the issue summary to first 3', () => {
