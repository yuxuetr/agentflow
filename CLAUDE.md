...
#### L2 — agentflow-mcp
Model Context Protocol integration: client + server, JSON-RPC 2.0, retry/timeout/reconnect, latency benchmarks. Dual-era on both sides (`docs/RFC_MCP_PROTOCOL_MODERNIZATION.md`, all 4 phases closed): client has two transports, `StdioTransport` (Legacy, `2024-11-05`, `initialize()` handshake) and `StreamableHttpTransport` (Modern, `2026-07-28`, stateless per-request `_meta`, hand-rolled per-request-scoped SSE parsing, W5.8-3) — `MCPClient::connect()` selects era by transport type (`client/era.rs`, W5.8-4; per-request `_meta` injection, `UnsupportedProtocolVersionError` retry, MRTR `InputRequiredResult` detection on the Modern path). Server mirrors the split: the Beta-pinned stdio surface (`MCPServer::handle_request`, closed 4-method set + `server/discover` added additively in W5.8-6) is unchanged, and a new Experimental `server_streamable_http` module (W5.8-7) exposes the same `handle_request` over `POST /` with `MCP-Protocol-Version`/`Mcp-Method`/`Mcp-Name` header validation. `agentflow-mcp/tests/streamable_http_server.rs` is the first real end-to-end round trip between this crate's own client and server (previously tested only in isolation), which caught and fixed two live wire-shape bugs: `ToolCall` only accepted a `parameters` params key (spec/client send `arguments`) and `ToolDefinition` serialized `input_schema` instead of the spec's `inputSchema`. The MCP→`agentflow-tool::Tool` adapter (`McpToolAdapter` + `McpClientPool`) lives in `agentflow-skills/src/mcp_tools.rs`, not in this crate — `agentflow-skills` owns the conversion because the skill builder is the entry point that knows which MCP servers a skill manifest declares.
