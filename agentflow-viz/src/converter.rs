...
impl WorkflowConverter {
            ...
            for dep_id in &node_def.dependencies {
                let edge = if node_def.run_if.is_some() {
                    VisualEdge::new(dep_id, &node_def.id)
                        .with_type(EdgeType::Conditional)
                } else {
                    VisualEdge::new(dep_id, &node_def.id)
                };
                graph.add_edge(edge);
            ...
            for (_input_name, mapping) in &node_def.input_mapping {
                ...
                if let Some(source_node) = Self::parse_node_reference(mapping) {
                    ...
                    if !node_def.dependencies.contains(&source_node) {
                        let edge = VisualEdge::new(&source_node, &node_def.id)
                            .with_type(EdgeType::DataFlow);
                        graph.add_edge(edge);
                    }
                }
            }
    ...
    fn convert_node(node_def: &NodeDefinition) -> VisualNode {
        ...
        // Add properties
        visual_node.properties.insert("type".to_string(), node_def.node_type.clone());
        
        if let Some(ref run_if) = node_def.run_if {
            visual_node.properties.insert("run_if".to_string(), run_if.clone());
        }
    ...
    fn parse_node_type(type_str: &str, parameters: &HashMap<String, serde_json::Value>) -> VisualNodeType {
...
"#;
    ...
    fn test_parse_node_reference() {
        ...
        assert_eq!(
            WorkflowConverter::parse_node_reference("some_value"),
            None
        );
    }
...
"#;
        ...
        assert!(matches!(map_node.node_type, VisualNodeType::Map { parallel: true }));
        
        let while_node = graph.get_node("while_node").unwrap();
        assert!(matches!(while_node.node_type, VisualNodeType::While { max_iterations: 50 }));
    }
}
