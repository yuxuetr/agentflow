...
async fn main() {
  ...
  let result = match cli.command {
    Commands::Workflow { command } => match command {
      ...
      WorkflowCommands::Validate { workflow_file } => {
        workflow::validate::execute(workflow_file).await
      }

      WorkflowCommands::List { list_type } => workflow::list::execute(list_type).await,
    },
