...
//! ## Run normally
//! ```bash
//! cargo run --example production_workflow --features observability
//! ```
//!
...
//! cargo run --example production_workflow --features observability -- --resume
//! ```
//!
//! ## Production mode with stricter timeouts
//! ```bash
//! ENV=production cargo run --example production_workflow --features observability
//! ```
...
use agentflow_core::{
    ...
    RetryPolicy, RetryStrategy, retry_executor::execute_with_retry,
    Result,
};
...
async fn setup_health_checks(config: &WorkflowConfig) -> Result<Arc<HealthChecker>> {
    ...
    checker.add_check("metrics", || {
        Box::pin(async {
            #[cfg(feature = "metrics")]
            {
                use agentflow_core::metrics::METRICS_ENABLED;
    ...
    checker.add_check("workflow_state", move || {
...
#[cfg_attr(feature = "observability", instrument(skip(health_checker, checkpoint_manager, resource_manager)))]
async fn run_workflow(
    ...
    if !health.is_healthy {
        let unhealthy_count = health.checks.iter()
            .filter(|c| matches!(c.status, agentflow_core::health::HealthStatus::Unhealthy))
            .count();
    ...
    checkpoint_manager.save_checkpoint_with_status(
        &config.workflow_id,
        "completed",
        &state,
...
#[cfg_attr(feature = "observability", instrument(skip(_health_checker, checkpoint, checkpoint_manager, resource_manager)))]
