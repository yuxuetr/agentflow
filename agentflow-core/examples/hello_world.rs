use agentflow_core::{SharedState, Node, Flow, Result};
...
impl Node for GreetingNode {
    fn prep(&self, shared: &SharedState) -> Result<Value> {
        let user_name = shared.get("user_name")
...
fn main() -> Result<()> {
    ...
    shared.insert("user_name".to_string(), Value::String("Rust Developer".to_string()));
