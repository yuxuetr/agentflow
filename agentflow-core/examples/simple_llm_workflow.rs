...
/// Simple demo of the code-first approach
#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  println!("🚀 AgentFlow Core - Simple LLM Workflow Demo\n");
  ...
  println!("📝 Inputs set in SharedState:");
  for (key, value) in shared_state.iter() {
    println!("   {}: {:?}", key, value);
  }
  println!();

  // Create LLM node with template
  let llm_node = LlmNode::new("answer_question", "{{ model }}")
    .with_prompt("{{ question }}")
    .with_system("You are a helpful AI assistant. Provide clear, concise, and accurate answers.")
    .with_temperature(0.7)
    .with_max_tokens(500);

  // Execute the workflow
  println!("▶️  Executing LLM Node...\n");
  match llm_node.run_async(&shared_state).await {
    Ok(_) => {
      println!("\n✅ Workflow completed successfully!");

      // Display results
      if let Some(answer) = shared_state.get("answer") {
        println!("\n🎯 Final Answer: {}", answer.as_str().unwrap_or("N/A"));
      }
    }
    Err(e) => {
      println!("\n❌ Workflow failed: {:?}", e);
...
mod tests {
  ...
  #[tokio::test]
  async fn test_llm_node_basic_execution() {
    let shared_state = SharedState::new();
    shared_state.insert(
      "question".to_string(),
      Value::String("What is 2+2?".to_string()),
    ...
    let llm_node = LlmNode::new("test", "{{ model }}").with_prompt("{{ question }}");

    let result = llm_node.run_async(&shared_state).await;
    assert!(result.is_ok());

    // Check that answer was stored
    assert!(shared_state.contains_key("answer"));
    assert!(shared_state.contains_key("test_output"));
  }
  ...
  async fn test_template_resolution() {
    ...
    let llm_node = LlmNode::new("resolver", "{{ selected_model }}")
      .with_prompt("Answer this: {{ user_question }}")
  ...
  async fn test_chained_execution() {
    ...
    // First node
    let node1 = LlmNode::new("first_question", "{{ model }}").with_prompt("{{ question }}");

    let result1 = node1.run_async(&shared_state).await;
    assert!(result1.is_ok());

    // Second node that uses the output of the first
    let node2 = LlmNode::new("follow_up", "{{ model }}")
      .with_prompt("Based on this answer: {{ answer }}, provide more details.");

    let result2 = node2.run_async(&shared_state).await;
    assert!(result2.is_ok());

    // Both outputs should be stored
    assert!(shared_state.contains_key("first_question_output"));
    assert!(shared_state.contains_key("follow_up_output"));
  }
}
