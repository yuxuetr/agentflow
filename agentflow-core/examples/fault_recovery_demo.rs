...
//! ```bash
//! cargo run --example fault_recovery_demo
//! ```
//!
//! ## Second Run (will resume from checkpoint)
//! ```bash
//! cargo run --example fault_recovery_demo -- --resume
//! ```
//!
...
async fn run_fresh_workflow(
  ...
  checkpoint_manager
    .save_checkpoint_with_status(workflow_id, "completed", &state, WorkflowStatus::Completed)
    .await?;
