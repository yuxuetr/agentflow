...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let nodes = vec![
    ...
    GraphNode {
      ...
      input_mapping: Some(
        vec![(
          "result".to_string(),
          ("node1".to_string(), "result".to_string()),
        )]
        .into_iter()
        .collect(),
      ),
    ...
    GraphNode {
      ...
      input_mapping: Some(
        vec![(
          "result".to_string(),
          ("node2".to_string(), "result".to_string()),
        )]
        .into_iter()
        .collect(),
      ),
    ...
    GraphNode {
      ...
      input_mapping: Some(
        vec![(
          "result".to_string(),
          ("node3".to_string(), "result".to_string()),
        )]
        .into_iter()
        .collect(),
      ),
  ...
  let flow = Flow::new(nodes.clone()).with_checkpointing(checkpoint_config)?;
  ...
  let _fixed_flow = Flow::new(fixed_nodes).with_checkpointing(checkpoint_config_resume)?;
