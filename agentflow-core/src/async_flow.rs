...
impl AsyncFlow {
  ...
  pub async fn run_async(&self, shared: &SharedState) -> Result<Value> {
    ...
    if let Some(ref collector) = self.metrics_collector {
      let event = ExecutionEvent {
        node_id: flow_name.to_string(),
        event_type: if result.is_ok() { "flow_success" } else { "flow_error" }.to_string(),
        timestamp: start_time,
      ...
      collector.increment_counter(&format!("{}.duration_ms", flow_name), duration.as_millis() as f64);
      if result.is_ok() {
  ...
  async fn run_async_internal(&self, shared: &SharedState) -> Result<Value> {
    ...
    if !self.parallel_nodes.is_empty() {
      ...
      return Ok(Value::String(format!("parallel_completed_{}", results.len())));
    }
    ...
    loop {
      ...
      if execution_count > MAX_EXECUTIONS {
        return Err(AgentFlowError::FlowExecutionFailed {
          message: format!("Flow execution exceeded maximum iterations ({})", MAX_EXECUTIONS),
        });
      }
      ...
      let action = match self.timeout {
        Some(timeout_duration) => {
          match tokio::time::timeout(timeout_duration, 
            current_node.run_async_with_observability(shared, self.metrics_collector.clone())).await {
            Ok(result) => result?,
            Err(_) => {
        ...
        None => current_node.run_async_with_observability(shared, self.metrics_collector.clone()).await?,
      };
      ...
      match next_node_id {
        Some(node_id) => {
          current_node = self.nodes.get(&node_id).ok_or_else(|| {
  ...
  pub async fn run_parallel(&self, nodes: Vec<&dyn AsyncNode>, shared: &SharedState) -> Result<Vec<Value>> {
    if nodes.is_empty() {
      return Ok(Vec::new());
    }
    ...
    let futures = nodes.iter().map(|node| async move {
      match self.timeout {
        Some(timeout_duration) => {
          match tokio::time::timeout(timeout_duration, 
            node.run_async_with_observability(shared, self.metrics_collector.clone())).await {
        ...
        None => node.run_async_with_observability(shared, self.metrics_collector.clone()).await.map(|r| Value::String(r.unwrap_or_default())),
      }
    });
  ...
  pub async fn run_batch(&self, nodes: Vec<&dyn AsyncNode>, shared: &SharedState, batch_size: usize) -> Result<Vec<Value>> {
    if nodes.is_empty() {
      return Ok(Vec::new());
    }
  ...
  pub async fn run_concurrent_batches(&self, nodes: Vec<&dyn AsyncNode>, shared: &SharedState) -> Result<Vec<Value>> {
    ...
    for batch_group in batches.chunks(max_concurrent) {
      let batch_futures = batch_group.iter().map(|batch| {
        self.run_parallel(batch.clone(), shared)
      });
...
mod tests {
  ...
  impl AsyncNode for SimpleAsyncNode {
    ...
    async fn exec_async(&self, _prep_result: Value) -> Result<Value> {
      ...
      {
        self.execution_log.lock().unwrap().push((self.id.clone(), std::time::Instant::now()));
      }
  ...
  impl AsyncNode for ConditionalAsyncNode {
    ...
    async fn post_async(&self, shared: &SharedState, _prep_result: Value, _exec_result: Value) -> Result<Option<String>> {
      let condition_value = shared.get(&self.condition_key)
    ...
    async fn exec_async(&self, prep_result: Value) -> Result<Value> {
      if let Value::Array(items) = prep_result {
        ...
        for batch in items.chunks(self.batch_size) {
          batch_count += 1;
          {
            self.execution_log.lock().unwrap().push(format!("batch_{}_executed", batch_count));
          }
  ...
  impl AsyncNode for NestedFlowNode {
    ...
    async fn exec_async(&self, _prep_result: Value) -> Result<Value> {
      ...
      {
        self.execution_log.lock().unwrap().push("nested_executed".to_string());
      }
  ...
  async fn test_async_flow_parallel_execution() {
    ...
    let parallel_nodes = (0..3).map(|i| SimpleAsyncNode {
    ...
    let flow = AsyncFlow::new_parallel(
      parallel_nodes.into_iter().map(|n| Box::new(n) as Box<dyn AsyncNode>).collect()
    );
  ...
  async fn test_async_flow_batch_processing() {
    ...
    let items: Vec<Value> = (0..15).map(|i| Value::Number(serde_json::Number::from(i))).collect();
    ...
    let processed_batches = shared.get("processed_batches")
      .and_then(|v| v.as_u64())
      .unwrap_or(0);
  ...
  async fn test_async_flow_backpressure() {
    ...
    let items: Vec<Value> = (0..20).map(|i| Value::Number(serde_json::Number::from(i))).collect();
    shared.insert("batch_items".to_string(), Value::Array(items));
