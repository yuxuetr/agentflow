...
| Crate | 职责 |
|---|---|
| **agentflow-tracing** | 可观测性：`EventListener`（非侵入事件采集，按到达顺序排空避免竞态）；持久化 JSONL（默认，`FileTraceStorage`，唯一已落地的存储后端——SQLite/Postgres 只有 DDL 常量，无实现，见 `docs/TRACING_USAGE.md`）；`trace replay` + TUI 时间线；OpenTelemetry span 模型（`OtelSpan` / `OtelSpanSink`）+ W3C trace context 传播；`otlp-http` feature 下 `OtlpHttpSpanSink` 提供首方 OTLP/HTTP+JSON 导出器（W4.4，基于官方 `opentelemetry-otlp` crate）；密钥/敏感参数脱敏；`AGENTFLOW_TRACE_DIR` / `AGENTFLOW_RUN_DIR` 存储根。OTLP gRPC 传输仍未实现，运维需要 gRPC 时仍需自带 `OtelSpanSink`。 |
