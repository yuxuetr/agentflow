...
| --- | --- | --- |
| MCP server (`agentflow-mcp::server`) | Beta | Closed method set `initialize` / `notifications/initialized` / `tools/list` / `tools/call`; new methods may be added in minor releases, existing methods stay wire-stable. Required response fields: `initialize` → `result.protocolVersion` + `result.capabilities` + `result.serverInfo.{name,version}`; `tools/list` → `result.tools[]` with per-item `{name, description, inputSchema}` (W5.8-8: corrected from a stale `input_schema` — the server always serialized snake_case, silently incompatible with any spec-compliant client, including this crate's own `MCPClient`, until the first real client↔server test caught it); `tools/call` success → `result.content`, failure → `error.{code,message}` envelope; `tools/call` request params accept both `arguments` (spec-standard, W5.8-8) and the original `parameters` key. Notifications return no response. Error codes: `-32601` method-not-found, `-32603` tool-execution-failed (mirrors JSON-RPC + the broader `JsonRpcErrorCode` enum). `STABLE_PROTOCOL_VERSION = "2024-11-05"` — bumping breaks Beta. `MCPServer::handle_request` is the single public entry point; transports beyond stdio drive it directly. Promoted to Beta after P10.5.2 added fixture-based compat tests in `agentflow-mcp/tests/server_contracts.rs`. `server/discover` (W5.8-6) is additive to this closed set but is itself Experimental — see the Streamable HTTP row below. |
| MCP Streamable HTTP server endpoint (`agentflow-mcp::server_streamable_http`) | Experimental | Modern-era (`2026-07-28`) dual-era companion to the Beta stdio row above, added W5.8-7 (`docs/RFC_MCP_PROTOCOL_MODERNIZATION.md` Phase 3). `POST /` dispatches through the same `MCPServer::handle_request` the stdio loop drives; the existing four Beta methods are unaffected. Requires `MCP-Protocol-Version: 2026-07-28` (else `-32022 UnsupportedProtocolVersionError` listing what's supported), `Mcp-Method` matching the body's `method` (else `-32020 HeaderMismatch`), and — for `tools/call` only — `Mcp-Name` matching `params.name` (else `HeaderMismatch`). Responses are always a single JSON object (`200`, or `202` empty body for a notification) — never SSE, since `MCPServerHandler` has no mid-call progress-notification mechanism today. `server/discover` (W5.8-6, reachable from both this endpoint and stdio) returns `result.supportedVersions` (currently `["2024-11-05", "2026-07-28"]`), `result.capabilities`, `result.serverInfo`. Zero external validation beyond this session's own tests (`agentflow-mcp/tests/streamable_http_server.rs`, including an end-to-end round trip against this crate's own `StreamableHttpTransport` client) — pin the AgentFlow version if depending on the header-validation error shapes. |
...
| `HarnessEvent` envelope | Beta | `agentflow-harness` line-delimited JSON: `seq`, `session_id`, `ts`, `kind`, `payload`. Closed kind set (`session_started`, `step_started`, `tool_call_requested`, `approval_requested`, `approval_decided`, `tool_call_completed`, `token_delta`, `interrupt_requested`, `interrupt_answered`, `background_task_updated`, `memory_summary_added`, `stopped`). Wire schema version `harness/1`. Additive optional fields and additive kinds keep the version; breaking changes bump it. Promoted to Beta after P-H.2 wired the envelopes through the in-process hook runtime and P-H.5 slice 2 surfaced them over `/v1/harness/sessions/{id}/events` (SSE) and `/events/history` (JSON). V2.2 added `token_delta` as an additive, live-only kind (never in `/events/history` or `AgentRunResult.events`) — wire version stays `harness/1`. V2.3 added `interrupt_requested`/`interrupt_answered` (additive, same posture) plus the `awaiting_input` `StopReason` variant inside `stopped`/`StoppedPayload`. |
...
| `AgentLoopCheckpoint` envelope | Experimental | `agentflow-agent-spi::checkpoint`, concrete file storage in `agentflow-agents::checkpoint::FileLoopCheckpointer` (`schema_version = 1`). Per-turn (ReAct) / per-plan-step (PlanExecute) snapshot of in-flight agent-loop progress, distinct from the `ResumePlan`/DAG-level `Flow` checkpoint row above. New in V2.4 (this release); no fixture-based compat tests yet — pin the AgentFlow version if depending on the on-disk shape. `runtime_kind` (`react` / `plan_execute`) discriminates the two producers; PlanExecute additionally carries `plan_steps`/`plan_position`/`observations`, empty/zero for ReAct checkpoints. Consumed by `agentflow harness resume-loop` (CLI) and `ReActAgent`/`PlanExecuteAgent::resume_from_loop_checkpoint`. |
| Distributed worker control plane (`WorkerProtocol`, `WorkerControlPlane`, `AuthenticatedControlPlane`, `WorkerCredential`, `WorkerAdmissionPolicy`, `JwtPolicy`, `WorkerCapabilities`, `ClaimHints`, `NodeExecutionPayload`) | Experimental | `agentflow-server::scheduler` exposes the worker transport (gRPC), admission policy (allowlist + per-worker pre-shared keys + per-worker JWT identity with HS256/RS256 + key rotation + fleet / per-worker concurrency caps), capability + locality dispatch hints (P10.16.2 — in-memory protocol only; gRPC wire-extension is a tracked follow-up), and the portable `NodeExecutionPayload` schema as of P2.8/P5.5/P10.16.1/P10.16.2. Wire shape, admission semantics, and the supported-`node_type` table are subject to additive changes until the surface graduates to Beta (target: end of N10). gRPC-metadata propagation of admission tokens is still deferred — pin the worker minor version to avoid surprises. See `docs/DISTRIBUTED.md` for the full contract. `AdmissionError::InvalidCredential` gained a `reason: String` field in P10.16.1 to surface the verifier-specific failure mode; existing exhaustive matches need a new arm. `WorkerTask` gained an optional `node_type` and `WorkerHeartbeat` gained `capabilities: WorkerCapabilities` in P10.16.2 — both default to empty so legacy callers are unaffected. T0.2 adds `WorkerAdmissionPolicy::for_profile` and a new `AdmissionConfigError` enum (startup-time fail-closed validation under `SecurityProfile::Production`, distinct from the per-call `AdmissionError`); existing exhaustive matches over `agentflow-server::scheduler` error types need a new arm only if they also match on this new type — `AdmissionError` itself is unchanged. |
...
| MCP server wire contract | Beta | `agentflow-mcp` | `agentflow-mcp/tests/fixtures/server_contracts/` | `initialize` / `notifications/initialized` / `tools/list` / `tools/call` request/response shapes, `STABLE_PROTOCOL_VERSION` round-trip, success vs error envelopes, JSON-RPC error codes (`-32601` / `-32603`), additive-field tolerance. `server/discover` (Experimental, W5.8-6) shares this fixture format but not the Beta promise. The Streamable HTTP endpoint's header-validation + client↔server interop (Experimental, W5.8-7/8) is covered separately by `agentflow-mcp/tests/streamable_http_server.rs` — not fixture-file-driven, since it exercises real HTTP requests rather than pinning `handle_request`'s in-process shape. |
