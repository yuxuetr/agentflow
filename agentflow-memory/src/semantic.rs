...
impl SemanticMemory {
    ...
    pub async fn open<P: AsRef<Path>>(
        ...
        let store = Self { pool, embedder, window_tokens };
    ...
    async fn add_message(&mut self, message: Message) -> Result<(), MemoryError> {
        ...
        if should_embed {
            match self.embedder.embed_text(&message.content).await {
                Ok(vector) => {
                    if let Err(e) =
                        self.insert_embedding(&message_id, &session_id, &vector).await
                    {
                        tracing::warn!(
    ...
    async fn search(
        ...
        match self.embedder.embed_text(query).await {
            Ok(query_vec) => {
                ...
                let rows = sqlx::query(
                    "SELECT message_id, vector FROM embeddings WHERE session_id = ?1",
                )
                .bind(session_id)
                .fetch_all(&self.pool)
                .await
                ...
                if rows.is_empty() {
                    tracing::debug!(
                        "No embeddings for session {session_id} — using keyword search"
                    );
...
fn row_to_message(row: &SqliteRow) -> Result<Message, MemoryError> {
    let id_str: String =
        row.try_get("id").map_err(|e| MemoryError::StorageError(e.to_string()))?;
    let session_id: String =
        row.try_get("session_id").map_err(|e| MemoryError::StorageError(e.to_string()))?;
    let role_str: String =
        row.try_get("role").map_err(|e| MemoryError::StorageError(e.to_string()))?;
    let content: String =
        row.try_get("content").map_err(|e| MemoryError::StorageError(e.to_string()))?;
    let ts_str: String =
        row.try_get("timestamp").map_err(|e| MemoryError::StorageError(e.to_string()))?;
    let tool_name: Option<String> =
        row.try_get("tool_name").map_err(|e| MemoryError::StorageError(e.to_string()))?;
    let token_count: i64 =
        row.try_get("token_count").map_err(|e| MemoryError::StorageError(e.to_string()))?;
    ...
    Ok(Message { id, session_id, role, content, timestamp, tool_name, token_count: token_count as u32 })
}
...
mod tests {
    ...
    impl FixedEmbedding {
        fn new(dim: usize) -> Self {
            Self { dim, call_count: Arc::new(AtomicUsize::new(0)), fail_with: None }
        }
    ...
    impl EmbeddingProvider for FixedEmbedding {
        ...
        fn model_name(&self) -> &str { "fixed-test-embedding" }
    }
    ...
    async fn add_and_retrieve_messages() {
        ...
        mem.add_message(Message::user(sid, "Hello, world!")).await.unwrap();
        mem.add_message(Message::assistant(sid, "Hi there!")).await.unwrap();
    ...
    async fn system_messages_are_stored_but_not_embedded() {
        ...
        let mut mem = SemanticMemory::in_memory(embedder.clone(), 8_000).await.unwrap();

        mem.add_message(Message::system("s", "You are helpful.")).await.unwrap();
    ...
    async fn search_returns_up_to_limit_messages() {
        ...
        for i in 0..5 {
            mem.add_message(Message::user(sid, &format!("message {i}"))).await.unwrap();
        }
    ...
    async fn search_falls_back_to_keyword_when_embedding_fails() {
        ...
        let mut mem =
            SemanticMemory::in_memory(failing_embedder, 8_000).await.unwrap();
        let sid = "fallback-session";

        // Messages are stored even though embedding fails
        mem.add_message(Message::user(sid, "rust is fast")).await.unwrap();
        mem.add_message(Message::user(sid, "python is slow")).await.unwrap();
    ...
    async fn search_falls_back_to_keyword_when_no_embeddings_stored() {
        ...
        mem.add_message(Message::user(sid, "hello keyword")).await.unwrap();
        mem.add_message(Message::user(sid, "other message")).await.unwrap();
    ...
    async fn clear_session_removes_messages_and_embeddings() {
        ...
        mem.add_message(Message::user(sid, "to be cleared")).await.unwrap();
        ...
        let count: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM embeddings WHERE session_id = ?1",
        )
        .bind(sid)
        .fetch_one(&mem.pool)
        .await
    ...
    async fn prune_evicts_oldest_non_system_messages() {
        ...
        mem.add_message(Message::user(sid, "message one")).await.unwrap(); // ~3 tokens
        mem.add_message(Message::user(sid, "message two")).await.unwrap(); // ~3 tokens
        mem.add_message(Message::user(sid, "message three and more")).await.unwrap(); // ~6 tokens
    ...
    async fn session_token_count_matches_stored_messages() {
        ...
        mem.add_message(Message::assistant(sid, "world")).await.unwrap();
    ...
    async fn search_is_isolated_per_session() {
        ...
        mem.add_message(Message::user("sess-a", "alpha message")).await.unwrap();
        mem.add_message(Message::user("sess-b", "beta message")).await.unwrap();
