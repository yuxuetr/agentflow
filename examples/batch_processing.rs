...
impl AsyncNode for TranslateTextNode {
  async fn prep_async(&self, shared: &SharedState) -> Result<Value> {
    ...
    let text = shared.get("text")
      .and_then(|v| v.as_str().map(|s| s.to_string()))
      .unwrap_or_else(|| "(No text provided)".to_string());
    
    let languages: Vec<String> = shared.get("languages")
  ...
  async fn exec_async(&self, prep_result: Value) -> Result<Value> {
    ...
    println!("🤖 [EXEC] Starting batch translation for {} languages", languages.len());
    ...
    for language in languages {
      ...
      let task = tokio::spawn(async move {
        ...
        println!("  ✅ {} translation completed in {:?}", language, processing_time);
    ...
    println!("⚡ [EXEC] All translations completed in {:?}", total_duration);
  ...
  async fn post_async(&self, shared: &SharedState, _prep: Value, exec: Value) -> Result<Option<String>> {
    ...
    let output_dir = shared.get("output_dir")
      .and_then(|v| v.as_str().map(|s| s.to_string()))
      .unwrap_or_else(|| "translations".to_string());
    
    println!("💾 [POST] Saving {} translations to {}/", results.len(), output_dir);
    
    // Create output directory
    if let Err(e) = fs::create_dir_all(&output_dir).await {
      return Err(AgentFlowError::NodeExecutionFailed { message: format!("Failed to create output directory: {}", e) });
    }
    ...
    for result in results {
      ...
      if let Err(e) = fs::write(&filename, content).await {
        return Err(AgentFlowError::NodeExecutionFailed { message: format!("Failed to write translation file {}: {}", filename, e) });
      }
    ...
    shared.insert("translation_count".to_string(), Value::Number(results.len().into()));
    shared.insert("output_directory".to_string(), Value::String(output_dir.clone()));
    shared.insert("total_processing_time_ms".to_string(), 
...
This framework enables building production-grade AI applications with confidence."#;
  ...
  shared.insert("languages".to_string(), serde_json::json!([
    "Chinese", "Spanish", "Japanese", "German"
  ]));
  shared.insert("output_dir".to_string(), Value::String("translations".to_string()));
  ...
  match translation_flow.run_async(&shared).await {
    Ok(result) => {
      ...
      let translation_count = shared.get("translation_count")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
        
      let output_dir = shared.get("output_directory")
        .and_then(|v| v.as_str().map(|s| s.to_string()))
        .unwrap_or_else(|| "unknown".to_string());
        
      let processing_time = shared.get("total_processing_time_ms")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
...
mod tests {
  ...
  async fn test_translate_node_lifecycle() {
    ...
    shared.insert("languages".to_string(), serde_json::json!(["Spanish", "French"]));
  ...
  async fn test_batch_translation_flow() {
    ...
    shared.insert("output_dir".to_string(), Value::String("test_translations".to_string()));
