...
async fn call_llm_structured(prompt: &str, expected_format: &str) -> String {
  ...
  - "Practical Applications and Impact""#
    ...
    "styled_article" => {
      ...
      format!(
        r#"# Exploring Our Topic: A Journey of Discovery
...
```"#,
  ...
  async fn post_async(
    ...
    shared.insert(
      "outline_yaml".to_string(),
      Value::String(yaml_content.to_string()),
    );
    shared.insert("sections".to_string(), json!(sections));
    shared.insert(
      "outline".to_string(),
      Value::String(formatted_outline.clone()),
    );
    shared.insert(
      "outline_generation_time_ms".to_string(),
      Value::Number(generation_time.into()),
    );
...
impl AsyncNode for WriteContentNode {
  async fn prep_async(&self, shared: &SharedState) -> Result<Value> {
    ...
    let sections: Vec<String> = shared
      .get("sections")
      .and_then(|v| {
        v.as_array().map(|arr| {
          arr
...
- Include one brief example or analogy"#,
    ...
    let results_json: Vec<Value> = section_results
      .iter()
      .map(|(section, content, processing_time)| {
        json!({
          "section": section,
          "content": content,
  ...
  async fn post_async(
    ...
    shared.insert(
      "content_writing_time_ms".to_string(),
      Value::Number(total_time.into()),
    );

    println!(
      "💾 [POST] Compiled draft from {} sections",
      section_results.len()
    );
    println!("\n===== DRAFT CONTENT =====");
    println!("{}", draft);
...
impl AsyncNode for ApplyStyleNode {
  async fn prep_async(&self, shared: &SharedState) -> Result<Value> {
    ...
    println!(
      "🔍 [PREP] Applying style to draft ({} characters)",
      draft.len()
    );
...
- Include a strong opening and conclusion"#,
  ...
  async fn post_async(
    ...
    // Store in shared state
    shared.insert(
      "final_article".to_string(),
      Value::String(final_article.to_string()),
    );
    shared.insert(
      "styling_time_ms".to_string(),
      Value::Number(styling_time.into()),
    );
    shared.insert(
      "original_length".to_string(),
      Value::Number(original_length.into()),
    );
    shared.insert(
      "final_length".to_string(),
      Value::Number(final_length.into()),
    );

    println!(
      "💾 [POST] Final article ready ({} characters)",
      final_length
    );
    println!("\n===== FINAL ARTICLE =====");
...
async fn run_article_workflow_example(topic: &str) -> Result<()> {
  ...
  let outline_time = shared
    .get("outline_generation_time_ms")
    .and_then(|v| v.as_u64())
    .unwrap_or(0);
  let content_time = shared
    .get("content_writing_time_ms")
    .and_then(|v| v.as_u64())
    .unwrap_or(0);
  let styling_time = shared
    .get("styling_time_ms")
    .and_then(|v| v.as_u64())
    .unwrap_or(0);

  let outline_length = shared
    .get("outline")
    .map(|v| v.as_str().map(|s| s.len()).unwrap_or(0))
    .unwrap_or(0);
  let draft_length = shared
    .get("draft")
    .map(|v| v.as_str().map(|s| s.len()).unwrap_or(0))
    .unwrap_or(0);
  let final_length = shared
    .get("final_article")
    .map(|v| v.as_str().map(|s| s.len()).unwrap_or(0))
    .unwrap_or(0);
  ...
  println!("Topic: {}", topic);
  println!(
    "Outline Generation: {}ms ({} chars)",
    outline_time, outline_length
  );
  println!(
    "Content Writing: {}ms ({} chars)",
    content_time, draft_length
  );
  println!(
    "Style Application: {}ms ({} chars)",
    styling_time, final_length
  );
  println!("Total Workflow Time: {:?}", total_workflow_time);

  // Verify workflow completion
  assert!(
    shared.get("outline").is_some(),
    "Outline should be generated"
  );
  assert!(shared.get("draft").is_some(), "Draft should be written");
  assert!(
    shared.get("final_article").is_some(),
    "Final article should be styled"
  );
...
mod tests {
  ...
  async fn test_style_application() {
    ...
    shared.insert(
      "draft".to_string(),
      Value::String("Test draft content".to_string()),
    );
