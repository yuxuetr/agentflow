...
impl VendorConfigManager {
  ...
  pub async fn split_config(&self, source_config: &LLMConfig) -> Result<SplitResult> {
    ...
    fs::create_dir_all(&self.config_dir).await.map_err(|e| LLMError::ConfigurationError {
      message: format!("Failed to create config directory: {}", e),
    })?;
    
    let models_dir = self.config_dir.join("models");
    fs::create_dir_all(&models_dir).await.map_err(|e| LLMError::ConfigurationError {
    ...
    for (vendor, models) in &vendor_models {
      ...
      let vendor_config = VendorModelConfig { models: models.clone() };
      
      let yaml_content = serde_yaml::to_string(&vendor_config).map_err(|e| LLMError::ConfigurationError {
      ...
      fs::write(&vendor_file, full_content).await.map_err(|e| LLMError::ConfigurationError {
        message: format!("Failed to write {}: {}", vendor_file.display(), e),
      })?;

      info!("Created {} with {} models", vendor_file.display(), models.len());
      result.vendor_files.push(vendor_file);
      result.models_per_vendor.insert(vendor.clone(), models.len());
    }
    ...
    let main_config_path = self.config_dir.join("config.yml");
    let yaml_content = serde_yaml::to_string(&main_config).map_err(|e| LLMError::ConfigurationError {
    ...
    fs::write(&main_config_path, full_content).await.map_err(|e| LLMError::ConfigurationError {
  ...
  async fn load_vendor_models(&self, models_dir: &Path) -> Result<HashMap<String, ModelConfig>> {
    ...
    let mut entries = fs::read_dir(models_dir).await.map_err(|e| LLMError::ConfigurationError {
      message: format!("Failed to read models directory: {}", e),
    })?;

    while let Some(entry) = entries.next_entry().await.map_err(|e| LLMError::ConfigurationError {
      ...
      if path.extension().and_then(|s| s.to_str()) == Some("yml") {
        ...
        match self.load_vendor_file(&path).await {
          Ok(vendor_models) => {
            let vendor_name = path.file_stem()
              .and_then(|s| s.to_str())
              .unwrap_or("unknown");
  ...
  async fn load_vendor_file(&self, file_path: &Path) -> Result<HashMap<String, ModelConfig>> {
    let content = fs::read_to_string(file_path).await.map_err(|e| LLMError::ConfigurationError {
      message: format!("Failed to read vendor file: {}", e),
    })?;

    let vendor_config: VendorModelConfig = serde_yaml::from_str(&content).map_err(|e| LLMError::ConfigurationError {
  ...
  pub async fn update_vendor_models(&self, vendor: &str, models: HashMap<String, ModelConfig>) -> Result<()> {
    let models_dir = self.config_dir.join("models");
    fs::create_dir_all(&models_dir).await.map_err(|e| LLMError::ConfigurationError {
      message: format!("Failed to create models directory: {}", e),
    })?;
    ...
    info!("Updated {} with {} models", vendor_file.display(), vendor_config.models.len());
    Ok(())
  }
...
impl SplitResult {
  ...
  pub fn create_report(&self) -> String {
    ...
    report.push_str(&format!("💡 Average models per file: {:.1}\n", avg_models_per_file));
...
impl PerformanceComparison {
  ...
  pub async fn benchmark_loading(monolithic_path: &Path, split_config_dir: &Path) -> Result<LoadingBenchmark> {
    ...
    let monolithic_metadata = fs::metadata(monolithic_path).await.map_err(|e| LLMError::ConfigurationError {
    ...
    let mut entries = fs::read_dir(split_config_dir).await.map_err(|e| LLMError::ConfigurationError {
    ...
    while let Some(entry) = entries.next_entry().await.map_err(|e| LLMError::ConfigurationError {
      ...
      let metadata = entry.metadata().await.map_err(|e| LLMError::ConfigurationError {
    ...
    if models_dir.exists() {
      let mut models_entries = fs::read_dir(&models_dir).await.map_err(|e| LLMError::ConfigurationError {
        message: format!("Failed to read models directory: {}", e),
      })?;
      
      while let Some(entry) = models_entries.next_entry().await.map_err(|e| LLMError::ConfigurationError {
...
impl LoadingBenchmark {
  ...
  pub fn create_report(&self) -> String {
    ...
    if self.split_load_time <= self.monolithic_load_time * 2 {
      report.push_str("  ✅ Split configuration provides good organization with minimal performance impact\n");
    } else {
      report.push_str("  ⚠️  Split configuration has notable performance impact - consider lazy loading\n");
    }
    
    if self.monolithic_file_size > 50_000 { // 50KB
      report.push_str("  ✅ Monolithic file is large enough to benefit from splitting\n");
    } else {
      report.push_str("  ℹ️  Monolithic file is small - splitting may not provide significant benefits\n");
    }
