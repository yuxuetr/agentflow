...
impl ModelValidator {
  ...
  pub async fn validate_config(&self, config: &LLMConfig) -> ValidationResult {
    ...
    for (model_name, model_config) in &config.models {
      info!("Validating model: {} (vendor: {})", model_name, model_config.vendor);
  ...
  async fn validate_single_model(
    ...
    if vendor_config.is_none() || !vendor_config.as_ref().unwrap().supports_model_list {
      info!("Vendor {} doesn't support model listing, skipping validation for {}", model_config.vendor, model_name);
      return Ok(true);
    }
    ...
    self.fetcher.model_exists(&model_config.vendor, model_id_to_check).await
  }
  ...
  pub async fn suggest_similar_models(&self, target_model: &str, vendor: &str) -> Result<Vec<String>> {
    ...
    Ok(suggestions.into_iter().take(5).map(|(model, _)| model).collect())
  }
  ...
  pub fn create_report(&self, result: &ValidationResult) -> String {
    ...
    if !result.valid_models.is_empty() {
      report.push_str(&format!("✅ Valid Models ({}):\n", result.valid_models.len()));
    ...
    if !result.invalid_models.is_empty() {
      report.push_str(&format!("❌ Invalid Models ({}):\n", result.invalid_models.len()));
    ...
    if !result.unavailable_vendors.is_empty() {
      report.push_str(&format!("⚠️  Unavailable Vendors ({}):\n", result.unavailable_vendors.len()));
    ...
    report.push_str(&format!("Summary: {}/{} models validated successfully\n", 
      result.valid_models.len(), 
...
mod tests {
  ...
  fn test_similarity_calculation() {
    ...
    // Exact match
    assert!(validator.calculate_similarity("gpt-4", "gpt-4") > validator.calculate_similarity("gpt-4", "gpt-3"));
    
    // Partial match
    assert!(validator.calculate_similarity("gpt-4-turbo", "gpt-4") > validator.calculate_similarity("gpt-4-turbo", "claude"));
