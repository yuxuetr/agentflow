...
async fn test_step_asr_with_different_audio_formats(
  api_key: &str,
  ...
  let wav_result = test_audio_format(
    &client,
    api_key,
    wav_data,
  ...
  let mp3_result = test_audio_format(
    &client,
    api_key,
    mp3_data,
  ...
  let flac_result = test_audio_format(
    &client,
    api_key,
    flac_data,
    "sample_audio.flac".to_string(),
    "audio/flac",
  )
  .await;
  match flac_result {
  ...
  if small_file_size < 25 * 1024 * 1024 {
    // 25MB typical limit
...
async fn test_batch_asr_processing(api_key: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let handle = tokio::spawn(async move {
      ...
      if response.status().is_success() {
        ...
        let text = response_json
          ...
          .and_then(|t| t.as_str())
          .unwrap_or("");
        Ok::<(usize, String, String), Box<dyn std::error::Error + Send + Sync>>((
          i + 1,
          filename,
          text.to_string(),
        ))
  ...
  println!(
    "📤 Processing {} ASR requests in parallel...",
    audio_files.len()
  );
  let start_time = std::time::Instant::now();
