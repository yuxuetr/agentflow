...
async fn test_step_1x_medium_quick(api_key: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, image_data) in data.iter().enumerate() {
      if let Some(url) = image_data.get("url").and_then(|u| u.as_str()) {
        ...
        if image_response.status().is_success() {
          ...
          fs::write(&filename, &image_bytes).await?;
          println!(
            "💾 Downloaded image saved to: {} ({} bytes)",
            filename,
            image_bytes.len()
          );
  ...
  println!(
    "   Time per step: {:.2}ms",
    duration.as_millis() as f32 / request_body["steps"].as_f64().unwrap() as f32
  );
...
async fn test_step_2x_large_advanced(api_key: &str) -> Result<(), Box<dyn std::error::Error>> {
  ...
  println!(
    "   CFG Scale: {} (strong guidance)",
    request_body["cfg_scale"]
  );
  let start_time = std::time::Instant::now();
    ...
    for (i, image_data) in data.iter().enumerate() {
      if let Some(b64_json) = image_data.get("b64_json").and_then(|b| b.as_str()) {
        ...
        if image_bytes.len() >= 24 && &image_bytes[0..8] == b"\x89PNG\r\n\x1a\n" {
          ...
          println!(
            "   📊 File size ratio: {:.2} KB per megapixel",
...
async fn test_step_2x_large_with_style_reference(
  api_key: &str,
    ...
    if let Some(data) = response_json.get("data").and_then(|d| d.as_array()) {
      for (i, image_data) in data.iter().enumerate() {
        if let Some(b64_json) = image_data.get("b64_json").and_then(|b| b.as_str()) {
          ...
          fs::write(&filename, &image_bytes).await?;
          println!(
            "💾 Style-inspired image {} saved to: {} ({} bytes)",
            i + 1,
            filename,
            image_bytes.len()
          );
    ...
    if let Some(data) = base_json.get("data").and_then(|d| d.as_array()) {
      if let Some(image_data) = data.first() {
        if let Some(b64_json) = image_data.get("b64_json").and_then(|b| b.as_str()) {
          ...
          println!(
            "✅ Base image created: base_image_for_edit.png ({} bytes)",
            image_bytes.len()
          );
        }
      }
...
async fn test_batch_image_generation(api_key: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Some(data) = response_json.get("data").and_then(|d| d.as_array()) {
          if let Some(image_data) = data.first() {
            if let Some(b64_json) = image_data.get("b64_json").and_then(|b| b.as_str()) {
              ...
              fs::write(&filename, &image_bytes).await?;
              return Ok::<(usize, String, String, usize), Box<dyn std::error::Error + Send + Sync>>(
                (i + 1, filename, prompt, image_bytes.len()),
              );
            }
          }
        }
  ...
  println!(
    "📤 Processing {} image generation requests in parallel...",
    prompts.len()
  );
  let start_time = std::time::Instant::now();
  ...
  println!(
    ...
    duration / prompts.len() as u32
  );
  println!(
    "   Total generated data: {:.2} MB",
    total_size as f32 / 1024.0 / 1024.0
  );
  println!(
    "   Throughput: {:.2} images/minute",
    prompts.len() as f32 / duration.as_secs_f32() * 60.0
  );
