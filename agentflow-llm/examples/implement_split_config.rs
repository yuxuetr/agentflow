...
use agentflow_llm::{LLMConfig, VendorConfigManager, AgentFlow};
use std::path::Path;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  println!("   ✅ Loaded {} models from {} vendors", 
    config.models.len(), 
    config.models.values().map(|m| &m.vendor).collect::<std::collections::HashSet<_>>().len()
  );
  ...
  println!("   📁 Main config: {}", split_result.main_config_file.display());
  ...
  println!("   ✅ Loaded {} models in {:?}", loaded_config.models.len(), load_time);
  
  // Verify the loaded config matches the original
  if loaded_config.models.len() == config.models.len() {
    println!("   ✅ Model count matches original configuration");
  } else {
    println!("   ⚠️  Model count mismatch: {} vs {}", 
      loaded_config.models.len(), config.models.len());
  }
  ...
  manager.update_vendor_models("openai", openai_models).await?;
  ...
  println!("\n🧹 Cleanup");
  println!("========");
  println!("The split configuration has been created in: {}", split_config_dir);
  println!("You can now:");
...
fn show_production_usage_examples(config_dir: &str) {
  ...
  println!();
  println!("let manager = VendorConfigManager::new(\"{}\");", config_dir);
  ...
  println!("AgentFlow::init_with_split_config(\"{}\").await?;", config_dir);
...
async fn show_file_structure(config_dir: &str) -> Result<(), Box<dyn std::error::Error>> {
  ...
  if main_config.exists() {
    ...
    println!("├── config.yml ({} KB) - Main configuration (providers, defaults)", 
      metadata.len() / 1024);
  }
  ...
  if models_dir.exists() {
    ...
    for (i, (filename, size)) in files.iter().enumerate() {
      ...
      println!("    {} {} ({} KB)", connector, filename.to_string_lossy(), size / 1024);
    }
  }
