...
async fn test_step_tts_mini_fast(api_key: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Generation speed: {:.2} chars/sec", 
             request_body["input"].as_str().unwrap().len() as f32 / duration.as_secs_f32());
    println!("   Audio size: {} bytes", audio_data.len());
    println!("   Compression ratio: {:.2} bytes/char", 
    ...
    let is_valid_wav = audio_header.len() >= 12 && 
                       &audio_header[0..4] == b"RIFF" && 
                       &audio_header[8..12] == b"WAVE";
...
async fn test_step_tts_vivid_multilingual(api_key: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Language: {}", cantonese_request["voice_label"]["language"]);
    let start_time = std::time::Instant::now();
    ...
    println!("   Language: {}", sichuan_request["voice_label"]["language"]);
    let start_time = std::time::Instant::now();
...
async fn test_step_tts_vivid_advanced(api_key: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Input length: {} chars", advanced_request["input"].as_str().unwrap().len());
    ...
    println!("   Estimated audio duration: {:.1} seconds", audio_duration_estimate);
    println!("   Real-time factor: {:.2}x", duration.as_secs_f32() / audio_duration_estimate);
    ...
    for (i, text) in texts.iter().enumerate() {
        ...
        let handle = tokio::spawn(async move {
            ...
            if response.status().is_success() {
                ...
                Ok::<(usize, String, usize), Box<dyn std::error::Error + Send + Sync>>((i + 1, filename, audio_data.len()))
            } else {
