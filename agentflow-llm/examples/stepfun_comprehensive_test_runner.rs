    ...
    if let Ok(mut entries) = fs::read_dir(".").await {
      while let Ok(Some(entry)) = entries.next_entry().await {
        if let Ok(name) = entry.file_name().into_string() {
          if name.starts_with(&format!("{}_", example_name)) || name.starts_with("stepfun_") {
...
fn print_test_summary(results: &[TestResult]) {
    ...
    for result in results.iter().filter(|r| !r.success) {
      println!(
        ...
        result
          .error_message
          .as_ref()
          .map(|e| e.lines().next().unwrap_or("Unknown error"))
  ...
  if total_files > 0 {
    println!(
      "\n📁 Generated {} output files across all tests",
      total_files
    );
  }
  ...
  if failed == 0 {
    ...
    println!(
      "⚠️  {} test(s) failed - check the logs above for details",
      failed
    );
  }
}
