...
use agentflow_llm::{ConfigUpdater, LLMConfig, VendorConfigManager};
use std::path::Path;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  if Path::new(split_config_dir).exists() {
    ...
    println!(
      "   ⚠️  Split config directory not found: {}",
      split_config_dir
    );
  }
...
async fn update_monolithic_config(config_path: &str) -> Result<(), Box<dyn std::error::Error>> {
  ...
  if let Some(google_provider) = config.providers.get("google") {
    if google_provider.base_url.as_ref().map(|u| u.as_str())
      == Some("https://generativelanguage.googleapis.com/v1beta/openai")
    {
      println!("   ✅ Google base URL is correct");
    } else {
      println!(
        "   ⚠️  Google base URL needs updating: {:?}",
        google_provider.base_url
      );
    }
  }

  if let Some(anthropic_provider) = config.providers.get("anthropic") {
    if anthropic_provider.base_url.as_ref().map(|u| u.as_str())
      == Some("https://api.anthropic.com/v1")
    {
      println!("   ✅ Anthropic base URL is correct");
    } else {
      println!(
        "   ⚠️  Anthropic base URL needs updating: {:?}",
  ...
  if let Some(dashscope_provider) = config.providers.get("dashscope") {
    if dashscope_provider.base_url.as_ref().map(|u| u.as_str())
      == Some("https://dashscope.aliyuncs.com/compatible-mode/v1")
    {
      println!("   ✅ DashScope base URL is correct");
    } else {
      println!(
        "   ⚠️  DashScope base URL needs updating: {:?}",
        dashscope_provider.base_url
      );
    }
  }
...
async fn update_split_config(config_dir: &str) -> Result<(), Box<dyn std::error::Error>> {
  ...
  println!(
    "   📊 Loaded {} models from split configuration",
    config.models.len()
  );

  // Verify provider configurations
  let providers_correct = config
    .providers
    .get("google")
...
async fn regenerate_split_with_correct_urls(
  monolithic_path: &str,
  ...
  println!(
    "   📁 Regenerated {} vendor files",
    result.vendor_files.len()
  );
  ...
  for (provider_name, expected_url) in expected_urls {
    if let Some(provider) = config.providers.get(provider_name) {
      ...
      if actual_url == expected_url {
        ...
        println!(
          "   ❌ {}: {} (expected: {})",
          provider_name, actual_url, expected_url
        );
      }
    } else {
