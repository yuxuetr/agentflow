    ...
    if let Some(first_choice) = choices.first() {
      if let Some(message) = first_choice.get("message") {
        if let Some(content) = message.get("content").and_then(|c| c.as_str()) {
          ...
          let has_quicksort =
            content_lower.contains("快速排序") || content_lower.contains("quicksort");
        ...
        for line in chunk_str.lines() {
          if line.starts_with("data: ") {
            ...
            if data.trim() == "[DONE]" {
              ...
              println!(
                "   Average chunk size: {:.1} chars",
                if chunk_count > 0 {
                  full_content.len() as f32 / chunk_count as f32
                } else {
                  0.0
                }
              );
              ...
              let has_superposition =
                content_lower.contains("叠加") || content_lower.contains("superposition");
    ...
    if let Some(first_choice) = choices.first() {
      if let Some(message) = first_choice.get("message") {
        if let Some(content) = message.get("content").and_then(|c| c.as_str()) {
          ...
          let has_scattering =
            content_lower.contains("散射") || content_lower.contains("scattering");
