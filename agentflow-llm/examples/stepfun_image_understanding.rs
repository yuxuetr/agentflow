    ...
    if let Some(first_choice) = choices.first() {
      if let Some(message) = first_choice.get("message") {
        if let Some(content) = message.get("content").and_then(|c| c.as_str()) {
          ...
          let describes_nature = content_lower.contains("自然")
            || content_lower.contains("nature")
            || content_lower.contains("outdoor");
          let mentions_colors = content_lower.contains("绿")
            || content_lower.contains("蓝")
            || content_lower.contains("color")
            || content_lower.contains("颜色");
          let describes_composition = content_lower.contains("构图")
            || content_lower.contains("景")
            || content_lower.contains("view");
    ...
    if let Some(first_choice) = choices.first() {
      if let Some(message) = first_choice.get("message") {
        if let Some(content) = message.get("content").and_then(|c| c.as_str()) {
          ...
          let content_lower = content.to_lowercase();
          let analyzes_data = content_lower.contains("数据")
            || content_lower.contains("data")
            || content_lower.contains("点");
          let mentions_trend = content_lower.contains("趋势")
            || content_lower.contains("trend")
            || content_lower.contains("关系");
          let discusses_axes = content_lower.contains("轴")
            || content_lower.contains("坐标")
            || content_lower.contains("axis");
          let shows_insight = content_lower.contains("回归")
            || content_lower.contains("线性")
            || content_lower.contains("regression");
...
async fn test_step_1v_32k_detailed_analysis(
  api_key: &str,
    ...
    if let Some(first_choice) = choices.first() {
      if let Some(message) = first_choice.get("message") {
        if let Some(content) = message.get("content").and_then(|c| c.as_str()) {
          ...
          let content_lower = content.to_lowercase();
          let analyzes_scene = content_lower.contains("场景")
            || content_lower.contains("地点")
            || content_lower.contains("scene");
          let discusses_lighting = content_lower.contains("光线")
            || content_lower.contains("光")
            || content_lower.contains("lighting");
          let mentions_composition = content_lower.contains("构图")
            || content_lower.contains("视觉")
            || content_lower.contains("composition");
          let shows_technique = content_lower.contains("技巧")
            || content_lower.contains("摄影")
            || content_lower.contains("拍摄");
          let cultural_elements = content_lower.contains("人文")
            || content_lower.contains("社会")
            || content_lower.contains("文化");
    ...
    if let Some(first_choice) = choices.first() {
      if let Some(message) = first_choice.get("message") {
        if let Some(content) = message.get("content").and_then(|c| c.as_str()) {
          ...
          let content_lower = content.to_lowercase();
          let compares_images = content_lower.contains("比较")
            || content_lower.contains("difference")
            || content_lower.contains("对比");
          let identifies_diversity = content_lower.contains("多样")
            || content_lower.contains("差异")
            || content_lower.contains("不同");
          let shows_reasoning = content_lower.contains("因为")
            || content_lower.contains("原因")
            || content_lower.contains("重要");
          let integrates_analysis = content_lower.contains("综合")
            || content_lower.contains("整体")
            || content_lower.contains("总体");
