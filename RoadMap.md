...
- 分布式调度: 抽象 worker，通过 gRPC / NATS / Redis Streams 之一让 DAG 可分布式；明确 `agentflow-server` 是否进化为 control plane。进行中: `WorkerProtocol` 抽象与 gRPC 选型已落地，`docs/DISTRIBUTED.md` 已记录两 worker 部署形态。
