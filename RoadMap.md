...
- Plugin / Custom Node 体系: 评估 dlopen + abi_stable 与 WASM (wasmtime/wasmer) 两条路径，给出 ABI/lifecycle/permissions/signatures 设计文档；提交最小可用实现。Plugin marketplace 远程化基础闭环已完成：统一 Skill/Plugin remote manifest schema、read-only HTTP registry client、本地 artifact cache、可插拔签名校验、顶层 `agentflow marketplace` CLI 与 `docs/MARKETPLACE.md` 已落地；cached artifact 解包到 skill/plugin 安装目录留后续 handoff。
