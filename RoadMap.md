...
- 分布式调度: 抽象 worker，通过 gRPC / NATS / Redis Streams 之一让 DAG 可分布式；明确 `agentflow-server` 是否进化为 control plane。进行中: `WorkerProtocol` 抽象、gRPC 选型、`WorkerControlPlane` run-state/trace 聚合 façade、`agentflow-worker` 最小 runtime/binary、`StitchedWorkerTraceEvent` 跨 worker trace ordering 基础已落地，`docs/DISTRIBUTED.md` 已记录两 worker 部署形态；远程 tonic adapter 和 OTel exporter 映射仍待完成。
