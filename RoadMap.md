...
- `ShellTool`/`ScriptTool` 接入 macOS sandbox-exec / Linux seccomp + chroot 子集；`Tool` 增加 `requires_capabilities()` (`fs.read` / `fs.write` / `net` / `exec`) 和被三方裁剪后的 `effective_capabilities`。✅ (2026-05-04 / 2026-05-07)
- `agentflow-llm` 客户端在 HTTP 请求 headers 注入 `traceparent`，统一 `WorkflowRunId / AgentSessionId / ToolCallId` 属性命名，OTel 端到端连续。✅ (2026-05-07)
- `agentflow-rag/eval/`: 标注集 + Recall@K / MRR / nDCG 指标 + baseline 对比；新增 CLI 子命令 `agentflow rag eval <dataset>`。✅ (2026-05-07)
- `docs/SKILL_PERMISSIONS.md` 写明 SkillSecurity vs ToolPolicy vs CLI flag 三方决策合并算法；`agentflow skill inspect --explain-permissions` 展示一次实际运行的最终决策路径。✅ (2026-05-04)
...
- Plugin / Custom Node 体系（✅ 2026-05-09 基础闭环）: subprocess + JSON-RPC plugin PoC、manifest、lifecycle、sandbox bridge、`type: plugin` workflow YAML、`agentflow plugin install/list/inspect/uninstall` 已落地。Plugin marketplace 远程化基础闭环也已完成：统一 Skill/Plugin remote manifest schema、read-only HTTP registry client、本地 artifact cache、可插拔签名校验、顶层 `agentflow marketplace` CLI 与 `docs/MARKETPLACE.md` 已落地；`agentflow marketplace install` 已支持 verified cache 后安全解包 `.tar` / `.tar.gz` 并按 package type 安装到 Skill / Plugin runtime 根目录，`--cache-only` 保留离线准备流程。
- 分布式调度（✅ 2026-05-08 基础闭环）: 抽象 worker，通过 gRPC / NATS / Redis Streams 之一让 DAG 可分布式；明确 `agentflow-server` 是否进化为 control plane。`WorkerProtocol` 抽象、gRPC 选型、`WorkerControlPlane` run-state/trace 聚合 façade、`agentflow-worker` 最小 runtime/binary、`StitchedWorkerTraceEvent` 跨 worker trace ordering、`OtelSpan` 映射已落地，`docs/DISTRIBUTED.md` 已记录两 worker 部署形态；远程 tonic adapter / 真 DAG scheduler 接入 / 多进程 100+ 节点验收留后续执行任务。
- Web UI（✅ 2026-05-08）: React + Vite + TypeScript SPA `agentflow-ui/` 落地并通过 `agentflow-server` 内嵌 `/ui` 单进程交付；新增 `GET /v1/runs` run list、`GET /v1/runs/{id}/graph` agentflow-viz DAG/status、`GET /v1/runs/{id}/events/history` trace history，并复用 SSE 展示 DAG 实时状态、Agent steps、Tool decisions；TUI 保留作为 headless 替代。
- Agent SDK 文档化（✅ 2026-05-07）: `docs/AGENT_SDK.md` 落地，覆盖 `AgentRuntime` / `ReflectionStrategy` / `MemorySummaryBackend` / `AgentMemoryHook` / `Tool` / `MemoryStore` 扩展点；配套 `custom_runtime` / `custom_reflection` / `custom_memory_summary` 三个 mock-only 示例；核心扩展 trait rustdoc 补齐，`agentflow-agents` / `agentflow-tools` / `agentflow-memory` 的 `cargo doc --no-deps` 0 warning。`AgentStepKind` 暂保持 closed enum，文档明确该边界（开放枚举留待后续单独评估）。

验收标准:

- 至少一个独立仓库可以分发一个 plugin 节点，被 AgentFlow 加载并运行；权限/版本/签名校验有据可查。✅ 本地 out-of-tree plugin 安装/运行 + marketplace manifest/cache/verify 基础链路已落地。
- 分布式 DAG 在 2 worker 集群上能执行 100+ 节点 workflow，trace 仍能跨 worker 拼接。
- Web UI 能实时展示一次 hybrid run，并接管 trace replay 的可视化职责。✅ 2026-05-08 `/ui` 已接入 run list、DAG graph/status、event history replay 与 SSE 增量。
- 三方插件示例进入 CI 编译/烟测。✅ 2026-05-09 quality examples job 覆盖 `agentflow-echo-plugin` build、`plugin_host_demo`、CLI `type: plugin` workflow e2e、marketplace plugin package install e2e。
...
- Web UI 能实时展示 hybrid run 的 DAG / Agent / Tool 状态。✅ 2026-05-08 `/ui` 已落地。
